...
def create_model(learn_rate =.001,activation="relu",init='glorot_uniform', kernel_regularizer="l1", inputshape=(None,40)):
    # create model
    model = Sequential()
    model.add(BatchNormalization())
    model.add(Conv1D(280, 8, padding = "same", input_shape=(inputshape,1), kernel_initializer=init, kernel_regularizer=regularizers.l2(0.01)))  # input_dim=(20, )
...
def create_model_Mel(learn_rate =.001,activation="relu",init='glorot_uniform', kernel_regularizer="l1", inputshape=(None,128)):
    # create model
    model = Sequential()
    model.add(BatchNormalization())
    model.add(Conv1D(280, 8, padding = "same", input_shape=(inputshape,1), kernel_initializer=init, kernel_regularizer=regularizers.l2(0.01)))  # input_dim=(20, )
...
def create_model_RMS(learn_rate =.001,activation="relu",init='glorot_uniform', kernel_regularizer="l1", inputshape=(None,20)):
    # create model
    model = Sequential()
    model.add(BatchNormalization())
    model.add(Conv1D(280, 8, padding = "same", input_shape=(inputshape,1), kernel_initializer=init, kernel_regularizer=regularizers.l2(0.01)))  # input_dim=(20, )
...
def create_model_Chromashift(learn_rate =.001,activation="relu",init='glorot_uniform', kernel_regularizer="l1", inputshape=(None,12)):
    # create model
    model = Sequential()
    model.add(BatchNormalization())
...
def create_model_specCont(learn_rate =.001,activation="relu",init='glorot_uniform', kernel_regularizer="l1", inputshape=(None,10)):
    # create model
    model = Sequential()
    model.add(BatchNormalization())
    model.add(Conv1D(280, 8, padding="same", input_shape=(inputshape, 1), kernel_initializer=init,
...
def create_model_tonnetz(learn_rate =.001,activation="relu",init='glorot_uniform', kernel_regularizer="l1", inputshape=(None,6)):
    # create model
    model = Sequential()
    model.add(BatchNormalization())
    model.add(Conv1D(280, 8, padding="same", input_shape=(inputshape, 1), kernel_initializer=init,
...
def create_model_LPCRPLP(learn_rate =.001,activation="relu",init='glorot_uniform', kernel_regularizer="l1", inputshape=(None,13)):
    # create model
    model = Sequential()
    model.add(BatchNormalization())
    model.add(Conv1D(280, 8, padding="same", input_shape=(inputshape, 1), kernel_initializer=init,
    ...
    for person in [1,2,3]:
        ...
        for sound in sounds:
            ...
            for k_value in k_values:
                ...
                learn_rate = [.00001, .001, .1]
                regularizer = ["l2"] #["l1", "l2"]
                #dropout = [0.2, 0.4, 0.6] #[0.0, 0.2, 0.4, 0.6]
                param_grid = dict(activation=activation, epochs=epochs, batch_size=batches, learn_rate=learn_rate, init=init, kernel_regularizer=regularizer)#, dropout=dropout)  # optimizer=optimizers, , momentum=momentum
                ...
                print("Testing " + str(validUser) + " " + str(sound) + " " + str(TP) + " " + str(FN) + " " + str(FP) + " " + str(TN))
