...
def create_model(learn_rate =.001, activation="relu", init='glorot_uniform', kernel_regularizer="l1", dropout=0, input_dim=20):
    batch_size = 100
    # create model
    model = Sequential()
    ...
    model.add(Conv1D(1, kernel_size=1, activation=activation, kernel_regularizer=kernel_regularizer)) # input_dim=input_dim))  # input_dim=(20, )
    model.add(MaxPooling1D(pool_size=2, strides=2))
    #model.add(Dropout(dropout))
    model.add(Conv1D(1, kernel_size=1, activation=activation, kernel_regularizer=kernel_regularizer))
    model.add(Dense(64, kernel_initializer=init, activation=activation, kernel_regularizer=kernel_regularizer))
    #model.add(Dropout(dropout))
    model.add(Dense(1, kernel_initializer=init, activation=activation, kernel_regularizer=kernel_regularizer))
    '''
